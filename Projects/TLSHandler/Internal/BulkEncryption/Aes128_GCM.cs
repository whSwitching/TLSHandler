...
namespace TLSHandler.Internal.BulkEncryption
{
    class Aes128_GCM : IAeadBulkEncryption
    {
        ...
        public byte[] Decrypt(byte[] secret, byte[] key, byte[] iv, byte[] aad, byte[] associated = null)
        {
        ...
        public byte[] Encrypt(byte[] plain, byte[] key, byte[] iv, byte[] aad, byte[] associated = null)
        {
            if (key.Length * 8 != this.KeySize)
                throw new InvalidOperationException($"the given key has invalid size {key.Length * 8}, expect {this.KeySize}");
